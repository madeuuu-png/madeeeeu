...
export default function RecuperarCedula() {
  ...
  const handleBuscarCedula = async (): Promise<void> => {
    ...
    try {
      ...
      console.log("🔍 Buscando con apellido:", pApellido, "y nombre:", pNombre);

      // Buscar que el Nombre contenga AMBOS (apellido Y nombre)
      const { data, error } = await supabase
        .from("estudiantes")
        .select("NumDocumento, Nombre")
        .ilike("Nombre", `%${pApellido}%`)
      ...
      const nombreEncontrado = data[0].Nombre;
          ...
          <View style={styles.card}>
            ...
            >
              <Text style={styles.buttonText}>
                {cargando ? "Buscando..." : "Buscar mi cédula"}
              </Text>
